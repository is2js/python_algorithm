...
# 느낀점 :
# * 1. 양쪽으로 인접항고려시 우선방향 선택 + hash 헛개비range(N+2)추가하면서 초기화 후 조건을 통해 고려안되게
# * 2. - i-1 == 0 and  i == 1 로,, 인접항의 경우의 수를 명시해버리는 sense
# * 3. 인접한list의 slicing = [1,1] 할당센스..
# * 4. hash (counting정렬, 계수정렬)에서  n은 너무 많아 메모리 차지, 작동하는 lost 0은 적고 대부분이 가만히 있는 1 
# -> 다른 방법고려 
