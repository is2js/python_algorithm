...
# import bisect
# # - 해설1풀이: heapq 2개 + 갯수맞추며 & 맞추더라도 새로운값 삽입이후 각 중앙값 후보들은 대소비교 되도록 -> 항상 왼쪽그룹(최대힙)에서 pop해서 중앙값 구하기
# # * 1. heap2개를 쓰는 것보다. built-in library인 bisect를 이용하여 시복을 줄인체로 중앙값 그대로 출력해버린다.
# # idx탐색 by 이진탐색 -> .bisect == .bisect_right: insert_point  (삽입하면, 다음 value 덮어쓰니까 조심해야됨. )
# #                      .bisect_left: insert_point or same_value_index (삽입하면, 중복자리니까 그대로 유지됨 )
# # * 삽입정렬 by 이진탐색 -> .insort: return sorted array <inplace, 반환값은 None >

# array = []

# for x in data:
#     # 1. 들어오는 족족 inplace로 삽입정렬시킨다.
#     bisect.insort(array, x)
#     # 2. 바로 중앙값 출력한다. -> //2는 원래 2개 중 오른쪽 index를 의미한다.
#     if len(array)%2 ==0:
